...
Measured on orders it has never seen, VOLO agrees with them **66.4 percent** of
...
`masterlist/bg3-masterlist.json` covers 10,314 mods. 6,648 were
categorised from section headers modders wrote in their own orders,
1,643 from name patterns, 1,038 from a Nexus or mod.io listing,
17 from where their author's other catalogued mods sit, 175 inferred
from their neighbours, 34 from curated overrides, and 759 are not
categorised at all. 9,545 of the 10,314 sit on a divider position.
...
87 working orders is a small corpus, and it is the binding constraint on
...
759 mods have no category from any source. Almost all appeared in exactly one
